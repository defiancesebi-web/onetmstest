...
- **A controlled `<select>` needs more than `value` + `onChange`.** The native reset writes the DOM node directly, and because the element's `value` prop is unchanged across the failed-submit render, React's diff sees nothing to do and never restores it — so the select silently snaps back to its first option while every text input around it keeps its value. Pair each `<select>` with a ref and an effect that rewrites `ref.current.value` whenever the action state or the selected value changes. This was found by browser verification during Task 8, not by reading code.
